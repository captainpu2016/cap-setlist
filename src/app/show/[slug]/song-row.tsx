...
/** 把一般的 Apple Music 網址轉成官方嵌入式播放器網址，不需要 MusicKit API 或開發者帳號 */
function toAppleMusicEmbedUrl(url: string): string | null {
  try {
    const u = new URL(url);
...
export default function SongRow({ item, index }: { item: SetlistItemWithSong; index: number }) {
  const [expanded, setExpanded] = useState(false);

  const embedUrl = item.song?.apple_music_url ? toAppleMusicEmbedUrl(item.song.apple_music_url) : null;
  const canPreview = !item.is_placeholder && Boolean(embedUrl);

  return (
    <li className="rounded-md px-2 py-1 transition-colors hover:bg-stage-900/70 -mx-2">
      <div className="flex items-center gap-4 py-3">
        ...
        <div className="min-w-0 flex-1">
          {item.is_placeholder ? (
            ...
            >
              <p className="truncate font-medium text-paper transition-colors hover:text-marquee">
                {item.song?.title}
              </p>
      ...
      {expanded && embedUrl && (
        <div className="mb-3 overflow-hidden rounded-xl">
          <iframe
            title={`${item.song?.title} — Apple Music 試聽`}
